...
function DnD.PrepareLimits(ID, Place)
	...
	elseif Place.alignX == WIDGET_ALIGN_LOW_ABS then
		...
		LimitMax.posX = (ParentSize.sizeX - Place.sizeX - Padding[2]) * DnD.Screen.realSizeX /
			DnD.Screen.fullVirtualSizeX
	end
	...
	elseif Place.alignY == WIDGET_ALIGN_LOW_ABS then
		...
		LimitMax.posY = (ParentSize.sizeY - Place.sizeY - Padding[3]) * DnD.Screen.realSizeY /
			DnD.Screen.fullVirtualSizeY
	end
...
function DnD.StopDragging(fSuccess)
	...
	if fSuccess then
		...
		DnD.Widgets[DnD.Dragging].Initial = {
			X = DnD.Place.posX,
			Y = DnD.Place.posY,
			HX = DnD.Place.highPosX,
			HY = DnD.Place.highPosY
		}
	else
